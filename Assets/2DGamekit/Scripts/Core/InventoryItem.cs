    ...
    public class InventoryItem : MonoBehaviour, IDataPersister
    {
        ...
        void OnTriggerEnter2D(Collider2D other)
        {
            if (layers.Contains(other.gameObject))
            {
                ...
                // Notify mission manager about the item collection
                if (Mission.MissionManager.Instance != null)
                {
                    Mission.MissionManager.Instance.UpdateMissionProgress(inventoryKey);
                }
