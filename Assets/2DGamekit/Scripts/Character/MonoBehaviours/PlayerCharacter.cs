...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        protected bool m_MovementEnabled = true;
        ...
        {
            // If movement is disabled, don't allow any input-based movement
            if (!m_MovementEnabled && useInput)
            {
                m_MoveVector.x = Mathf.MoveTowards(m_MoveVector.x, 0f, groundDeceleration * Time.deltaTime);
                return;
            }
        ...
        {
            // If movement is disabled, gradually stop horizontal movement
            if (!m_MovementEnabled)
            {
                float deceleration = groundDeceleration * airborneDecelProportion;
                m_MoveVector.x = Mathf.MoveTowards(m_MoveVector.x, 0f, deceleration * Time.deltaTime);
                return;
            }
        ...
        {
            // Don't allow jumping if movement is disabled
            if (!m_MovementEnabled)
                return false;
        ...
        {
            // Don't allow input-triggered falling if movement is disabled
            if (!m_MovementEnabled)
                return false;
        ...
        public void DisableMovement()
        {
            ...
            // Stop the player from moving by zeroing out their movement vector
            SetMoveVector(Vector2.zero);
            // Also ensure PlayerInput doesn't affect the character
